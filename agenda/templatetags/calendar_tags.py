...
def date_tag(year, month, day):
    if day and int(day) != 0:
        ...
        filtered_events = Event.objects.filter(
            initial_date__lte=date, end_date__gte=date
        )
...
def next_month_tag(year, month):
    date_aux = datetime.date(
        year=int(year), month=int(month), day=28
    ) + datetime.timedelta(days=7)
    month_aux = date_aux.month
    year_aux = date_aux.year

    return reverse(
        "agenda:show_specific_calendar", kwargs={"year": year_aux, "month": month_aux}
    )
...
def next_year_tag(year):
    date_aux = datetime.date(year=int(year), month=12, day=31) + datetime.timedelta(
        days=1
    )
    year_aux = date_aux.year
...
def previous_year_tag(year):
    date_aux = datetime.date(year=int(year), month=1, day=1) - datetime.timedelta(
        days=1
    )
    year_aux = date_aux.year
...
def next_day_tag(year, month, day):
    ...
    return reverse(
        ...
        kwargs={"year": year_aux, "month": month_aux, "day": day_aux},
    )
