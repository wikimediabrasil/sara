...
class TeamArea(models.Model):
    text = models.CharField(
        _("Name"), max_length=420, help_text=_("Human-readable name of the team area")
    )
...
class Position(models.Model):
    text = models.CharField(
        _("Name"), max_length=420, help_text=_("Human-readable name of the position")
    )
...
class UserProfile(models.Model):
    ...
    professional_wiki_handle = models.CharField(
        _("Professional username"), max_length=50, blank=True, default=""
    )
    personal_wiki_handle = models.CharField(
        _("Personal username"), max_length=50, blank=True, default=""
    )
    photograph = models.CharField(
        _("Photograph filename"), max_length=420, blank=True, default=""
    )
    ...
    lattes = models.CharField(
        _("Lattes"), max_length=100, blank=True, default="", help_text=_("Lattes ID")
    )
    orcid = models.CharField(
        _("ORCID"), max_length=100, blank=True, default="", help_text=_("ORCID ID")
    )
    ...
    def __str__(self):
        return (
            self.professional_wiki_handle or self.user.first_name or self.user.username
        )
...
class UserPosition(models.Model):
    ...
    end_date = models.DateField(
        ...
        blank=True,
        help_text=_(
            "The end date for this user's position. Leave empty if this is the current position"
        ),
    )
    ...
    def __str__(self):
        if self.end_date:
            return _(
                "%(user_profile)s – %(position)s (%(start_date)s → %(end_date)s)"
            ) % {
